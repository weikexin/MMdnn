...
def get_kernel_extents(params):
    ko_h = (params.kernelSize[0]-1) * params.dilationFactor[0] + 1
    ko_w = (params.kernelSize[1]-1) * params.dilationFactor[1] + 1
    return ko_h, ko_w

def get_filter_output_shape(i_h, i_w, params):
    ko_h, ko_w = get_kernel_extents(params)

    o_h = (i_h + 2 * params.p_h - ko_h) / float(params.s_h) + 1
    o_w = (i_w + 2 * params.p_w - ko_w) / float(params.s_w) + 1
    return (int(math.ceil(o_h)), int(math.ceil(o_w)))
